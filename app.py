...
def get_key_by_torn_id(torn_id):
    ...
    with db() as con:
        return con.execute(
            'SELECT * FROM premium_keys WHERE owner_torn_id=? ORDER BY created_at DESC LIMIT 1',
            (str(torn_id),)
        ).fetchone()
...
def enabled_device_count(token):
    with db() as con:
        return con.execute(
            'SELECT COUNT(*) c FROM devices WHERE premium_key=? AND is_enabled=1',
            (token,)
        ).fetchone()['c']
...
def total_device_count(token):
    with db() as con:
        return con.execute(
            'SELECT COUNT(*) c FROM devices WHERE premium_key=?',
            (token,)
        ).fetchone()['c']
...
def public_key(row):
    ...
    return {
        ...
        'allowed_payment_amounts': [
            PREMIUM_KEY_PRICE_XANAX_30_DAYS * m
            for m in range(1, MAX_PAYMENT_MONTHS + 1)
        ],
...
def request_key():
    ...
    with db() as con:
        ...
        if existing:
            con.execute(
                'UPDATE premium_keys SET owner_name=COALESCE(NULLIF(?, ""), owner_name) WHERE premium_key=?',
                (owner_name, existing['premium_key'])
            )
            row = con.execute(
                'SELECT * FROM premium_keys WHERE premium_key=?',
    ...
    return jsonify({
        'ok': True,
        'message': 'Premium key reserved. Send payment to Fries91, then the scanner can activate/extend it.',
        'key': public_key(row)
    })
...
def payment_claim():
    ...
    if not months:
        ...
        return jsonify({
            'ok': False,
            'error': f'Payment must be one of: {", ".join(map(str, allowed))} Xanax.'
        }), 400
    ...
    if existing_key and not get_key(existing_key):
        return jsonify({
            'ok': False,
            'error': 'That premium key was not found. Use Get/reserve key first.'
        }), 404
    ...
    return jsonify({
        ...
        'message': 'Claim saved. Auto-scan can also activate payments found in Fries91 event logs.'
    })
...
def key_find():
    ...
    if not row:
        return jsonify({
            'ok': False,
            'error': 'No premium key found for that Torn ID. Use Get/reserve key first.'
        }), 404
...
def device_register():
    ...
    if not key_is_live(row):
        return jsonify({
            'ok': False,
            'error': 'Premium key is locked or expired. Send payment renewal first.'
        }), 402
    ...
    if not sub:
        return jsonify({
            'ok': False,
            'error': 'Missing push subscription. Browser did not give the app a push token.'
        }), 400

    sub_json = json.dumps(sub, sort_keys=True)
    now = iso(utcnow())
    device_name = (data.get('device_name') or row['owner_name'] or 'Phone').strip()

    with db() as con:
        existing = con.execute(
            'SELECT id, created_at FROM devices WHERE premium_key=? AND subscription_json=?',
            (token, sub_json)
        ).fetchone()

        if existing:
            con.execute(
                'UPDATE devices SET device_name=?, is_enabled=1, last_seen=? WHERE id=?',
...
def device_toggle():
    ...
    if total_device_count(token) <= 0:
        return jsonify({
            'ok': False,
            'error': 'No phone is registered to this key yet. Press Enable phone alerts first and allow notifications.'
        }), 404
    ...
    with db() as con:
        con.execute(
            'UPDATE devices SET is_enabled=?, last_seen=? WHERE premium_key=?',
            (enabled, iso(utcnow()), token)
        )
        con.commit()
...
def activate_or_extend_key(owner_name, owner_torn_id, amount, source='auto-scan'):
    ...
    with db() as con:
        ...
        if not row:
            ...
            ''', (
                token,
                owner_name or f'Torn {owner_torn_id}',
                owner_torn_id,
                'Premium banker ping key',
                iso(new_until),
                iso(now)
            ...
            con.execute(
                'UPDATE premium_keys SET paid_until=?, is_active=1, owner_name=COALESCE(NULLIF(?, ""), owner_name) WHERE premium_key=?',
                (iso(new_until), owner_name or '', token)
            )
...
def send_to_key(token, title, body, data):
    ...
    if not key_is_live(row):
        return {
            'ok': False,
            'error': 'Premium key is locked or expired',
            'status': 402
        }

    with db() as con:
        devices = con.execute(
            'SELECT * FROM devices WHERE premium_key=? AND is_enabled=1',
            (token,)
        ).fetchall()

        con.execute(
            'UPDATE premium_keys SET last_used=? WHERE premium_key=?',
            (iso(utcnow()), token)
        )
...
def banking_request():
    ...
    with db() as con:
        con.execute('''
            INSERT INTO alerts
            (id, premium_key, title, body, amount, requester_name, status, created_at, data_json)
            VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?)
        ''', (
            alert_id,
            token,
...
def parse_payment_event(text):
    ...
    return {
        'sender_torn_id': sender_id,
        'sender_name': sender_name or f'Torn {sender_id}',
        'amount': amount
    }
...
def scan_payments_once():
        ...
        try:
            ...
            with db() as con:
                for ev in events:
                    ...
                    key_row, days, created = activate_or_extend_key(
                        parsed['sender_name'],
                        parsed['sender_torn_id'],
                        parsed['amount'],
                        source='auto-scan'
                    )
                    ...
                    if key_row:
                        ...
                        ''', (
                            ...
                            now,
                            now,
                            f'Auto approved from Torn event {event_id}',
                            days,
                            1 if created else 0,
                            key_row['premium_key']
                        ))
                        ...
                        results.append({
                            ...
                            'torn_id': parsed['sender_torn_id'],
                            'amount': parsed['amount'],
                            'premium_key': key_row['premium_key'],
                            'days_added': days
                        })

                con.execute('''
                    INSERT INTO scan_runs
                    (ran_at, ok, events_seen, payments_found, payments_activated, message)
                    VALUES (?, 1, ?, ?, ?, ?)
                ''', (
                    now,
                    events_seen,
    ...
    return {
        ...
        'payments_activated': payments_activated,
        'message': message,
        'activated': results
    }
...
def admin_scan_status():
    ...
    with db() as con:
        runs = con.execute(
            'SELECT * FROM scan_runs ORDER BY ran_at DESC LIMIT 10'
        ).fetchall()

        events = con.execute(
            'SELECT * FROM processed_payment_events ORDER BY processed_at DESC LIMIT 25'
        ).fetchall()
    ...
    return jsonify({
        ...
        'auto_scan_payments': AUTO_SCAN_PAYMENTS,
        'scan_interval_seconds': SCAN_INTERVAL_SECONDS,
        'has_fries91_key': bool(FRIES91_TORN_API_KEY),
        'runs': [row_to_dict(r) for r in runs],
        'processed_events': [row_to_dict(e) for e in events]
    })
...
def admin_pending():
    ...
    with db() as con:
        rows = con.execute(
            'SELECT * FROM payment_claims WHERE status="pending" ORDER BY created_at ASC'
        ).fetchall()
...
def admin_approve():
    ...
    with db() as con:
        con.execute('''
            ...
            SET status='approved', reviewed_at=?, reviewed_by=?, admin_note=?, days_added=?, key_created=?, created_key=?
            WHERE id=?
        ''', (
            now,
            data.get('reviewed_by') or 'Fries91',
            data.get('admin_note') or '',
            days,
            1 if created else 0,
            key_row['premium_key'],
            claim_id
...
def admin_reject():
    ...
    with db() as con:
        con.execute('''
            UPDATE payment_claims
            SET status='rejected', reviewed_at=?, reviewed_by=?, admin_note=?
            WHERE id=? AND status='pending'
        ''', (
            iso(utcnow()),
            data.get('reviewed_by') or 'Fries91',
            data.get('admin_note') or 'Could not verify payment.',
            claim_id
        ))
...
def admin_keys():
    ...
    with db() as con:
        rows = con.execute(
            'SELECT * FROM premium_keys ORDER BY paid_until DESC'
        ).fetchall()

    return jsonify({
        'ok': True,
        'keys': [public_key(r) for r in rows]
    })
...
def admin_key_toggle():
    ...
    with db() as con:
        con.execute(
            'UPDATE premium_keys SET is_active=? WHERE premium_key=?',
            (enabled, token)
        )
    ...
    return jsonify({
        'ok': True,
        'key': public_key(row) if row else None
    })
